...
#include "mods/P2mTools.h" //ptr2plus
...
namespace FullscreenUI
{
	...
	static void DrawSettingsWindow(MainWindowType menu_type);
	...
	static void DrawIntListPresetSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section, const char* key,
		int default_value, const char* const* options, size_t option_count, bool translate_options, int option_offset = 0, bool enabled = true,
	...
	static void DrawIntListSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section, const char* key,
		...
		ImFont* summary_font = g_medium_font, bool preset_setting = false);
	...
	static void SetPresetToCustom(SettingsInterface* bsi);
	static void DrawPresetListSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section, const char* key,
		const char* default_value, const char* const* options, const char* const* option_values, size_t option_count, bool translate_options,
		bool enabled = true, float height = ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT, ImFont* font = g_large_font, ImFont* summary_font = g_medium_font,
		const char* translation_ctx = TR_CONTEXT);
	...
	static void DrawStringListSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section, const char* key,
		const char* default_value, const char* const* options, const char* const* option_values, size_t option_count, bool translate_options,
		bool enabled = true, float height = ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT, ImFont* font = g_large_font, ImFont* summary_font = g_medium_font,
		const char* translation_ctx = TR_CONTEXT, bool preset_list = false, bool preset_setting = false);
	
	static void DrawStringListPresetSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section, const char* key,
		const char* default_value, const char* const* options, const char* const* option_values, size_t option_count, bool translate_options,
		bool enabled = true, float height = ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT, ImFont* font = g_large_font, ImFont* summary_font = g_medium_font,
		const char* translation_ctx = TR_CONTEXT);
	
	static void SetPresetSettings(SettingsInterface* bsi, int index);
...
void FullscreenUI::DrawIntListSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section, const char* key,
	...
	float height, ImFont* font, ImFont* summary_font, bool preset_setting)
{
	...
	if (MenuButtonWithValue(title, summary, value_text, enabled, height, font, summary_font))
	{
		...
		OpenChoiceDialog(title, false, std::move(cd_options),
			[game_settings, section, key, option_offset, preset_setting](s32 index, const std::string& title, bool checked) {
				if (index >= 0)
				{
					auto lock = Host::GetSettingsLock();
					SettingsInterface* bsi = GetEditingSettingsInterface(game_settings);
					if (preset_setting && ( bsi->GetIntValue(section, key) != index + option_offset) )
					{
						SetPresetToCustom(bsi);
					}
					if (game_settings)
					{
						if (index == 0)
...
void FullscreenUI::DrawIntListPresetSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section, const char* key,
	int default_value, const char* const* options, size_t option_count, bool translate_options, int option_offset, bool enabled, float height, ImFont* font,
	ImFont* summary_font)
{
	FullscreenUI::DrawIntListSetting(bsi, title, summary, section, key,
		default_value, options, option_count, translate_options, option_offset, enabled, height, font,
		summary_font, true);
}
...
void FullscreenUI::DrawStringListSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section,
	...
	bool translate_options, bool enabled, float height, ImFont* font, ImFont* summary_font, const char* translation_ctx, bool preset_list, bool preset_setting)
{
	const bool game_settings = IsEditingGameSettings(bsi);
			...
			enabled, height, font, summary_font))
	{
		...
		OpenChoiceDialog(title, false, std::move(cd_options),
			[game_settings, section, key, option_values, preset_list, preset_setting](s32 index, const std::string& title, bool checked) {
				if (index >= 0)
				{
					auto lock = Host::GetSettingsLock();
					SettingsInterface* bsi = GetEditingSettingsInterface(game_settings);
					if (preset_setting && (bsi->GetStringValue(section, key) != option_values[index]))
						SetPresetToCustom(bsi);
					if (game_settings)
					{
						if (index == 0)
...
void FullscreenUI::DrawPresetListSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section,
	const char* key, const char* default_value, const char* const* options, const char* const* option_values, size_t option_count,
	bool translate_options, bool enabled, float height, ImFont* font, ImFont* summary_font, const char* translation_ctx)
{
	FullscreenUI::DrawStringListSetting(bsi, title, summary, section,
		 key,  default_value,  options,  option_values,  option_count,
		 translate_options, enabled, height, font, summary_font, translation_ctx, true);
}

void FullscreenUI::DrawStringListPresetSetting(SettingsInterface* bsi, const char* title, const char* summary, const char* section,
	const char* key, const char* default_value, const char* const* options, const char* const* option_values, size_t option_count,
	bool translate_options, bool enabled, float height, ImFont* font, ImFont* summary_font, const char* translation_ctx)
{
	FullscreenUI::DrawStringListSetting(bsi, title, summary, section,
		key, default_value, options, option_values, option_count,
		translate_options, enabled, height, font, summary_font, translation_ctx, false, true);
}

void FullscreenUI::SetPresetToCustom(SettingsInterface* bsi)
{
	bsi->SetStringValue("EmuCore", "performance_preset", "0");
}
 

void FullscreenUI::SetPresetSettings(SettingsInterface* bsi, int index)
{
	
	//could be a switch
	if (index == 2) // max quality
	{
		//affinity disabled
		bsi->SetIntValue("EmuCore/CPU", "AffinityControlMode", 0);
		//EE 100%
		bsi->SetIntValue("EmuCore/Speedhacks", "EECycleRate", 0);
		//EE Cycle Skipping Disabled
		bsi->SetIntValue("EmuCore/Speedhacks", "EECycleSkip", 0);
		//Optimal Frame ON
		bsi->SetIntValue("EmuCore/GS", "VsyncQueueSize", 0);
		// Blending Max
		bsi->SetIntValue("EmuCore/GS", "accurate_blending_unit", 5);

		// ani 16x
		bsi->SetStringValue("EmuCore/GS", "MaxAnisotropy", "16");
		// Mip FULL
		bsi->SetIntValue("EmuCore/GS", "mipmap_hw", 2);
		//Trilinear Forced
		bsi->SetIntValue("EmuCore/GS", "TriFilter", 2);
	}
	else if (index == 1) // balanced
	{
		//affinity disabled
		bsi->SetIntValue("EmuCore/CPU", "AffinityControlMode", 0);
		//EE 75% under
		bsi->SetIntValue("EmuCore/Speedhacks", "EECycleRate", -1);
		//EE Cycle Skipping Disabled
		bsi->SetIntValue("EmuCore/Speedhacks", "EECycleSkip", 0);
		//Optimal Frame Off
		//if (DEFAULT_FRAME_LATENCY < 2)
		//{
			bsi->SetIntValue("EmuCore/GS", "VsyncQueueSize", 2);
		//}
	    /* else
		{
			bsi->SetIntValue("EmuCore/GS", "VsyncQueueSize", DEFAULT_FRAME_LATENCY);
		}*/
		// Blending medium
		bsi->SetIntValue("EmuCore/GS", "accurate_blending_unit", 2);
		// ani 8x
		bsi->SetStringValue("EmuCore/GS", "MaxAnisotropy", "8");
		// Mip FULL
		bsi->SetIntValue("EmuCore/GS", "mipmap_hw", 2);
		//Trilinear Forced
		bsi->SetIntValue("EmuCore/GS", "TriFilter", 2);
	}
	else if (index == 0) // max performance
	{
		//affinity VU EE GS
		bsi->SetIntValue("EmuCore/CPU", "AffinityControlMode", 3);
		//EE 60% under
		bsi->SetIntValue("EmuCore/Speedhacks", "EECycleRate", -2);
		//EE Cycle Skipping Disabled
		bsi->SetIntValue("EmuCore/Speedhacks", "EECycleSkip", 0);
		//Optimal Frame Off
		//if (DEFAULT_FRAME_LATENCY < 2)
		...
		// Blending Minimum
		bsi->SetIntValue("EmuCore/GS", "accurate_blending_unit", 0);

		// ani OFF
		bsi->SetStringValue("EmuCore/GS", "MaxAnisotropy", "0");
		// Mip FULL
		bsi->SetIntValue("EmuCore/GS", "mipmap_hw", 2);
		//Trilinear Forced
		bsi->SetIntValue("EmuCore/GS", "TriFilter", 2);
			...
			ImVec2(0.0f, 0.0f), heading_size, "settings_category", ImVec4(UIPrimaryColor.x, UIPrimaryColor.y, UIPrimaryColor.z, bg_alpha)))
	{
		...
		if (!ImGui::IsPopupOpen(0u, ImGuiPopupFlags_AnyPopup))
		{
			if (ImGui::IsKeyPressed(ImGuiKey_GamepadDpadLeft, true) ||
				ImGui::IsKeyPressed(ImGuiKey_NavGamepadTweakSlow, true) ||
...
//draw navbar buttons and tab text
			...
			heading_pos, heading_size + navbar_size, "settings_category", ImVec4(UIPrimaryColor.x, UIPrimaryColor.y, UIPrimaryColor.z, 0.0f), 30.0f))
	{
		...
		if (!ImGui::IsPopupOpen(0u, ImGuiPopupFlags_AnyPopup))
		{
			//if L pressed then switch current page
			if (ImGui::IsKeyPressed(ImGuiKey_GamepadDpadLeft, true) ||
			...
			//if R pressed then switch current page
			else if (ImGui::IsKeyPressed(ImGuiKey_GamepadDpadRight, true) ||
					 ImGui::IsKeyPressed(ImGuiKey_NavGamepadTweakFast, true) ||
					 ImGui::IsKeyPressed(ImGuiKey_RightArrow, true))
			{
...
void FullscreenUI::DrawQuickSettingsPage()
{
	...
	static const char* s_preset_entries[] = {
		"Max Performance",
		"Balanced",
		"Max Quality",
		"Custom"
	...
	static const char* s_preset_values[] = {
		"1",
		"2",
		"3",
		"0"
	...
	BeginMenuButtons();
	MenuHeading("Performance");
	DrawPresetListSetting(bsi, "Performance Preset", "Adjusts multiple settings based on the selected preset.", "EmuCore", "performance_preset",
		"0", s_preset_entries, s_preset_values, std::size(s_preset_entries), false);
...
void FullscreenUI::DrawEmulationSettingsPage()
{
	...
	DrawIntListPresetSetting(bsi, FSUI_CSTR("EE Cycle Rate"), FSUI_CSTR("Underclocks or overclocks the emulated Emotion Engine CPU."), "EmuCore/Speedhacks",
		"EECycleRate", 0, ee_cycle_rate_settings, std::size(ee_cycle_rate_settings), true, -3);
	DrawIntListPresetSetting(bsi, FSUI_CSTR("EE Cycle Skipping"), FSUI_CSTR("Makes the emulated Emotion Engine skip cycles. Helps a small subset of games like SOTC. Most of the time it's harmful to performance."),
		"EmuCore/Speedhacks", "EECycleSkip", 0, ee_cycle_skip_settings, std::size(ee_cycle_skip_settings), true);
	//deleted/moved in upstream?
	//DrawIntListPresetSetting(bsi, "Affinity Control Mode",
	...
	DrawToggleSetting(bsi, FSUI_CSTR("Enable MTVU (Multi-Threaded VU1)"), FSUI_CSTR("Generally a speedup on CPUs with 4 or more cores. Safe for most games, but a few are incompatible and may hang."),
	...
	//	FSUI_CSTR("Reduces timeslicing between VU1 and EE recompilers, effectively running VU1 at an infinite clock speed."), "EmuCore/Speedhacks",
	//	"vu1Instant", true);
	DrawToggleSetting(bsi, FSUI_CSTR("Enable Cheats"), FSUI_CSTR("Enables loading cheats from pnach files."), "EmuCore", "EnableCheats", false);
	//deleted/moved in upstream?
	//DrawToggleSetting(bsi, FSUI_CSTR("Enable Host Filesystem"), FSUI_CSTR("Enables access to files from the host: namespace in the virtual machine."), "EmuCore",
	//	"HostFs", false, false);
...
//draw ptr2plus version text at top
	std::string ptr2plus_text = "PTR2+ Alpha 1.2";
...
//draw pause menu buttons
			...
			ImVec2(10.0f, 10.0f), ImGuiWindowFlags_NoBackground))
	{
		...
		BeginMenuButtons(submenu_item_count[static_cast<u32>(s_current_pause_submenu)], 1.0f, ImGuiFullscreen::LAYOUT_MENU_BUTTON_X_PADDING,
			ImGuiFullscreen::LAYOUT_MENU_BUTTON_Y_PADDING, ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY, true, pause_image_height);
